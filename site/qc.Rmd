...
if (file.exists(PLOTS)) {
  ...
  fwrite(dt, file = "../site_tables/01_variant_summary.tsv", sep="\t")
} else {
  dt <- fread( "../site_tables/01_variant_summary.tsv")
}
...
if (file.exists(PLOTS)) {
  ...
  fwrite(dt_sex_removal, file = "../site_tables/02_sex_removal_summary.tsv", sep="\t")
} else {
  dt_sex_removal <- fread("../site_tables/02_sex_removal_summary.tsv")
}

dt_sex_removal %>% mutate("%" = color_bar("#0081c2")(unlist(dt_sex_removal[,'%']))) %>% 
  kable("html", escape=FALSE, align=c('l', 'r', 'r', 'r', 'r'), format.args = list(big.mark = ",")) %>% row_spec(0,bold=TRUE) %>%
  row_spec(nrow(dt_sex_removal), bold = T, color = "white", background = "#0C6EBE") %>% 
  kable_styling('hover')
```
...
knitr::include_graphics("./plots/10_PC1_PC2_classify_EUR_loose_1kg_labelled.jpg")
knitr::include_graphics("./plots/10_PC3_PC4_classify_EUR_loose_1kg_labelled.jpg")
knitr::include_graphics("./plots/10_PC5_PC6_classify_EUR_loose_1kg_labelled.jpg")
```

And here are the resultant classifications after imposing a probability cutoff of `r T_European_RF`.

```{r pca_1kg, echo=FALSE, out.width = '100%', warning=FALSE}
knitr::include_graphics("./plots/10_PC1_PC2_classify_EUR_strict.jpg")
knitr::include_graphics("./plots/10_PC3_PC4_classify_EUR_strict.jpg")
knitr::include_graphics("./plots/10_PC5_PC6_classify_EUR_strict.jpg")
```
