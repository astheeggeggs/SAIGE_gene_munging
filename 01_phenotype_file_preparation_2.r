...
TRANCHE <- '200k'
cts_cat <- c("cts", "binary")

# Tables to write
output_folder <- paste0("/well/lindgren/UKBIOBANK/dpalmer/ukb_wes_phenotypes/", TRANCHE)
filtered_output <- paste0(output_folder, '/UKBB_WES', TRANCHE , "_filtered_", cts_cat, "_dec2021_phenotypes.tsv")
filtered_output_imp <- paste0(output_folder, '/UKBB_WES', TRANCHE , "_filtered_", cts_cat, "_dec2021_phenotypes_with_imputed_genos.tsv")  

# Tables to merge
# Quantitative and binary phenotypes
file <- paste0("/well/lindgren/UKBIOBANK/dpalmer/ukb_wes_phenotypes/curated_phenotypes_", cts_cat, ".tsv")
...
for (i in 1:length(file)) {
    ...
    withdrawn <- fread("/well/lindgren-ukbb/projects/ukbb-11867/DATA/QC/w11867_20220222.csv") %>% transmute(eid = V1)

    dt <- dt[which(!dt$eid %in% withdrawn$eid), ]

    fwrite(dt, file=filtered_output[i], sep='\t')
    ...
    fwrite(dt, file=filtered_output_imp[i], sep='\t')
    system(paste("bgzip", filtered_output_imp[i]))
}